    ...
    for i, smi in enumerate(smiles):
        if smi != '---':
            try:
                ...
                fps[i] = rdMolDescriptors.GetMorganFingerprintAsBitVect(mol, 2, nBits=2048)
            except:
                mols[i] = None
                fps[i] = None
        ...
        for i, smi in enumerate(target_smiles):
            if smi != '---':
                try:
                    ...
                    target_mols[i] = None
        target_fps = [rdMolDescriptors.GetMorganFingerprintAsBitVect(mol, 2, nBits=2048) for mol in target_mols if mol is not None]
            ...
            if fp is not None and target_fp is not None:
                try:
                    ...
                    try:
                        ...
                        target_fp = rdMolDescriptors.GetMorganFingerprintAsBitVect(mol, 2, nBits=2048)
                        similarity[i, j] = metric(fp, target_fp)
                    except:
                        similarity[i, j] = 0
    return similarity
...
class Evaluator:
    ...
    def test_model(self, save=True, eval_index: int=0):
        ...
        for target in self.config.test_ids + self.config.train_ids:
            ...
            smiles, _ = self.model_runner.targetted_generation(target, repeat=self.config.budget, batch_size=self.config.batch_size)
            result_df = self.evaluate(smiles, target=target, calc_similarity=True, calc_molprops=True)
